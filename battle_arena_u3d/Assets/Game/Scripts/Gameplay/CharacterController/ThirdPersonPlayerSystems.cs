    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var (playerInputs, player) in SystemAPI.Query<RefRW<ThirdPersonPlayerInputs>, ThirdPersonPlayer>().WithAll<Simulate>())
        {
            if (SystemAPI.HasComponent<ThirdPersonCharacterControl>(player.ControlledCharacter) && SystemAPI.HasComponent<CharacterStateMachine>(player.ControlledCharacter))
            {
                ...
                CharacterState characterState = SystemAPI.GetComponent<CharacterState>(player.ControlledCharacter);
                ...
                characterControl.Attack = characterState.IntervalAttack > 0f ? false : playerInputs.ValueRW.AttackPressed;
                characterControl.Skill = characterState.IntervalSkill > 0f ? false : playerInputs.ValueRW.SkillPressed;
